...
type SalesInvoiceHeader = SalesOrder;
...
function normalizeInvoiceHeader(order: SalesInvoiceHeader): SalesOrder {
  return {
    ...order,
...
export async function getSalesOrdersWithCount(
  ...
  const response = await apiGet<ODataResponse<SalesInvoiceHeader>>(endpoint);
...
export async function getSalesOrderByNo(
  ...
  return value && value.length > 0 ? normalizeInvoiceHeader(value[0]) : null;
}
