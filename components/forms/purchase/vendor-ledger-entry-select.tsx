...
/**
 * VendorLedgerEntrySelect component for Purchase forms
...
import { getVendorLedgerEntriesForDialog, type VendorLedgerEntry } from "@/lib/api/services/vendor-ledger.service";

interface VendorLedgerEntrySelectProps {
  value: string;
  vendorNo?: string;
  onChange: (value: string, entry?: VendorLedgerEntry) => void;
  placeholder?: string;
  disabled?: boolean;
  className?: string;
  hasError?: boolean;
...
const OPTIONAL_COLUMNS: ColumnConfig[] = [
  { id: "Vendor_No", label: "Vendor No.", sortable: true, filterType: "text", width: "120px" },
  { id: "VendorName", label: "Vendor Name", sortable: true, filterType: "text", flex: true },
  { id: "Description", label: "Description", sortable: true, filterType: "text", width: "200px" },
  { id: "Amount", label: "Amount", sortable: true, filterType: "text", width: "120px" },
  { id: "Remaining_Amount", label: "Remaining Amt", sortable: true, filterType: "text", width: "120px" },
  { id: "Currency_Code", label: "Currency", sortable: true, filterType: "text", width: "100px" },
];
...
export function VendorLedgerEntrySelect({
  ...
  const [vendors, setVendors] = useState<VendorLedgerEntry[]>([]);
  ...
  useEffect(() => {
    ...
    const found = vendors.find((v) => getEntryValue(v) === value);
    if (found) setDisplayLabel(`${found.Document_No} - ${found.External_Document_No} (${found.Posting_Date})`);
  ...
  const handleFilter = (colId: string, val: string) => {
    ...
    // Resolves the selected string to display value
  const getEntryValue = (entry: VendorLedgerEntry) => {
    return entry.External_Document_No?.trim() || entry.Document_No?.trim() || String(entry.Entry_No);
  };

  const handleSelect = (vendor: VendorLedgerEntry) => {
    onChange(getEntryValue(vendor), vendor);
    setOpen(false);
  };
                ...
                {loading ? (
                  ...
                  (() => {
                    ...
                    const selectedVendor = value
                      ? vendors.find((v) => getEntryValue(v) === value)
                      : null;
                    const restVendors = selectedVendor
                      ? vendors.filter((v) => getEntryValue(v) !== value)
                      : vendors;
                      ...
                      return (
                        ...
                        >
                          ...
                          {currentColumns.map((col) => {
                            ...
                            if (col.id === "Document_No" || col.id === "External_Document_No") {
                              return (
