...
export function ForgotPasswordForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await forgotPassword(
        formData.userID,
        formData.registredModileNo,
      );

      // Check if request was successful
      if (
        response.value !== "Password has been sent your registered mobile no."
      ) {
        setError(
          "Failed to send password. Please check your UserID and mobile number.",
        );
  ...
  if (success) {
    return (
      <div
        className={cn(
          "flex flex-col gap-4 rounded-md border border-green-200 bg-green-50 p-4 dark:border-green-800 dark:bg-green-950",
          className,
        )}
      >
  ...
  return (
    ...
    >
      ...
      <FieldGroup>
        ...
        <Field>
          <FieldLabel htmlFor="forgot-mobile">
          ...
          <Input
            ...
            required
          />
          {validationErrors.registredModileNo &&
            validationErrors.registredModileNo.length > 0 && (
              <p className="text-destructive mt-1 text-sm">
