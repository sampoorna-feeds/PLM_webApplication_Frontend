...
/**
 * Purchase Item Tracking Dialog
 * Assign / edit / delete lot tracking on a purchase order line.
 * Mirrors SalesItemTrackingDialog but uses purchase-orders.service functions.
 */
...
import {
  assignPurchaseItemTracking,
  getPurchaseItemTrackingLines,
  deletePurchaseItemTrackingLine,
  type PurchaseLine,
  type PurchaseItemTrackingLine,
...
export function PurchaseItemTrackingDialog({
  ...
  useEffect(() => {
    if (open && line && itemNo && locationCode) {
      ...
      const fetchTrackingLines = async () => {
        ...
        try {
          const lines = await getPurchaseItemTrackingLines(
            orderNo,
            lineNo,
            itemNo,
            locationCode,
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      await assignPurchaseItemTracking({
        orderNo,
        lineNo,
        itemNo,
        locationCode,
  ...
  const handleEditLine = (l: PurchaseItemTrackingLine) => {
    setEditingLine(l);
    setLotNo(l.Lot_No || "");
  ...
  const handleDeleteLine = (l: PurchaseItemTrackingLine) => {
    setLineToDelete(l);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deletePurchaseItemTrackingLine(
        ...
        <DialogContent className="flex max-h-[90vh] max-w-[95vw] flex-col gap-0 p-0 sm:max-w-[85vw] md:max-w-4xl lg:max-w-6xl">
          <DialogHeader className="border-b px-4 py-4 sm:px-6 md:px-8">
            <DialogTitle className="text-lg">
              {editingLine
                ? "Edit Item Tracking (Purchase Line)"
                : "Assign Item Tracking (Purchase Line)"}
            </DialogTitle>
          </DialogHeader>
