...
export type PurchaseInvoiceStatusTab =
  | "Open"
  | "Pending Approval"
  | "Released"
  | "";
...
export interface UsePurchaseInvoicesOptions {
  ...
  statusFilter?: PurchaseInvoiceStatusTab;
}
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      const result = searchQuery
        ? await searchPurchaseInvoices({
            ...commonParams,
            searchTerm: searchQuery,
          })
        : await getPurchaseInvoicesWithCount(commonParams);

      setOrders(result.orders);
      setTotalCount(result.totalCount);
    } catch (error) {
      console.error("Error fetching purchase invoices:", error);
      toast.error("Failed to load purchase invoices. Please try again.");
      setOrders([]);
      setTotalCount(0);
    } finally {
      setIsLoading(false);
