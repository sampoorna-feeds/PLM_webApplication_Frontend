...
import {
  usePurchaseReturnOrders,
  type PurchaseReturnOrderStatusTab,
} from "./use-purchase-return-orders";

export interface PurchaseReturnOrderViewProps {
  /** Status filter for tab (Open | Pending Approval | Released). Applied at API level. */
  statusFilter?: PurchaseReturnOrderStatusTab;
  /** Called when return order is created (e.g. to refetch list). Passed to FormStack context. */
  onPlaceOrder?: () => void;
  /** Optional: register refetch so parent can trigger list refresh */
  registerRefetch?: (refetch: () => void) => void;
}

export function PurchaseReturnOrderView({
  statusFilter,
  onPlaceOrder,
  registerRefetch,
}: PurchaseReturnOrderViewProps) {
  ...
  } = usePurchaseReturnOrders({ statusFilter });
