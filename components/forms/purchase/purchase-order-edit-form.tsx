 ...
 * Single-page layout: top = order header, middle = line items list (add/edit), bottom = changes summary + Update.
...
function purchaseLineToLineItem(
  line: PurchaseLine,
  _orderNo: string,
): LineItem {
...
export function PurchaseOrderEditForm({
  ...
  const [order, setOrder] = useState<PurchaseOrder | null>(
    () => (initialFormData?.order as PurchaseOrder) ?? null,
  );
  ...
  const [shipToCode, setShipToCode] = useState(
    () =>
      ...
      (initialFormData?.order as PurchaseOrder)?.Ship_to_Code ??
      "",
  );
  ...
  useEffect(() => {
    ...
    if (initialFormData?.[PERSIST_KEY] && initialFormData?.order) {
      if (!hasRestoredRef.current) {
        ...
        setOrder(initialFormData.order as PurchaseOrder);
    ...
    Promise.all([getPurchaseOrderByNo(orderNo), getPurchaseOrderLines(orderNo)])
      .then(([header, lines]) => {
        ...
        const items = lines.map((l) => purchaseLineToLineItem(l, orderNo));
  ...
  const handleAddLineItem = useCallback(() => {
    ...
    const tabIdNew = openTab("line-item", {
      title: "Add Line Item",
      formData: {
        customerNo: order.Buy_from_Vendor_No,
        locationCode,
        customerPriceGroup: "",
      },
      context: {
