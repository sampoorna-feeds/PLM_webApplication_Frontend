...
export type PurchaseReturnOrderStatusTab =
  | "Open"
  | "Pending Approval"
  | "Released"
  | "";

export interface UsePurchaseReturnOrdersOptions {
  /** Status filter for tab (Open | Pending Approval | Released). Applied at API level. */
  statusFilter?: PurchaseReturnOrderStatusTab;
}

export function usePurchaseReturnOrders(options: UsePurchaseReturnOrdersOptions = {}) {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      const result = searchQuery
        ? await searchPurchaseReturnOrders({
            ...commonParams,
            searchTerm: searchQuery,
          })
        : await getPurchaseReturnOrdersWithCount(commonParams);

      setOrders(result.orders);
      setTotalCount(result.totalCount);
    } catch (error) {
      console.error("Error fetching purchase return orders:", error);
      toast.error("Failed to load purchase return orders. Please try again.");
      setOrders([]);
      setTotalCount(0);
    } finally {
      setIsLoading(false);
