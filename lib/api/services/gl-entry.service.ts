...
/**
 * Get total debit and credit sums for a GL Account based on current filters
 */
export async function getGLEntrySums(
  filters: GLEntryFilters
): Promise<{ debitSum: number; creditSum: number }> {
  if (!filters.accountNo) return { debitSum: 0, creditSum: 0 };
  ...
  try {
    ...
    console.warn("Aggregation failed for GL Entry Sums, falling back", error);
    ...
    if (!response?.value || response.value.length === 0) {
      console.warn("No data returned for manual GL sum fallback");
    ...
    console.log(`Manually summing ${response.value.length} GL entries. Keys:`, keys, "DKey:", dKey, "CKey:", cKey);
