...
export async function searchOutwardGateEntries(
  ...
  if (!searchTerm || searchTerm.trim() === "") {
    return getOutwardGateEntriesWithCount(params);
  }
  ...
  const responses = await Promise.all(
    fieldsToSearch.map((field) => {
      ...
      const filter = rest.$filter
        ...
        : filterPart;
      return getOutwardGateEntriesWithCount({ ...rest, $filter: filter, $top: limit });
    }),
  );
...
export async function postOutwardGateEntry(docNo: string, userId?: string): Promise<string> {
  const encodedCompany = encodeURIComponent(COMPANY);
