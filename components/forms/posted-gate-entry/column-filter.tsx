...
import type { Column } from "./column-config";
import { DateInput } from "@/components/ui/date-input";

interface PostedGateEntryColumnFilterProps {
  column: Column;
  value: string;
  valueTo?: string;
...
export function PostedGateEntryColumnFilter({
  column,
  value,
  valueTo,
  onChange,
}: PostedGateEntryColumnFilterProps) {
  ...
  useEffect(() => {
    ...
    if (column.type === "number" && value.includes(":")) {
      const [op, val] = value.split(":");
      setNumberOperator(op as any);
      setLocalValue(val);
    }
  }, [value, valueTo, column.type]);

  const handleApply = () => {
    if (
      column.type === "date" ||
      (column.type === "number" && numberOperator === "range")
    ) {
