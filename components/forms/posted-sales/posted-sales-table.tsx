...
export function PostedSalesTable({
  ...
  return (
    <div className="rounded-md border bg-card shadow-sm flex flex-col flex-1 min-h-0 w-full overflow-hidden">
      <Table containerRef={scrollContainerRef} containerClassName="flex-1 min-h-0 overflow-auto">
        <TableHeader className="sticky top-0 z-10 bg-muted shadow-sm">
          <TableRow className="hover:bg-muted/50 border-b">
            {onPrint && (
              <TableHead className="h-10 px-4 py-2 text-center text-[11px] font-bold uppercase text-muted-foreground">
                Actions
              </TableHead>
            )}
        ...
        <TableBody>
          {isLoading ? (
              ...
              <TableRow key={i} className="border-b last:border-0">
                {onPrint && (
                  <TableCell className="p-4">
                    <Skeleton className="mx-auto h-4 w-8 opacity-50" />
                  </TableCell>
                )}
                {activeColumns.map((col) => (
            ...
            <TableRow>
              <TableCell colSpan={activeColumns.length + (onPrint ? 1 : 0)} className="h-24 text-center text-sm italic text-muted-foreground">
            ...
            <>
              ...
              {!isLoading && (
                <TableRow ref={sentinelRef}>
                  <TableCell colSpan={activeColumns.length + (onPrint ? 1 : 0)} className="p-0 border-0 bg-transparent">
