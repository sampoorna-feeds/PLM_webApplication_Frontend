...
export function VoucherForm() {
  ...
  const [showUpdateVoucherWarning, setShowUpdateVoucherWarning] =
    useState(false);
  const [isEditingVoucher, setIsEditingVoucher] = useState(false);
  // Track which voucher line numbers failed (for highlighting)
  const [failedVoucherLineNos, setFailedVoucherLineNos] = useState<Set<number>>(
    new Set(),
  );
  ...
  const [loadingTaxDetails, setLoadingTaxDetails] = useState<
  ...
  const showAccountTds =
    accountType === "Vendor" &&
    formData.accountNo &&
    accountTdsSections.length > 0;
  const showAccountTcs =
  ...
  const showBalanceTds =
    formData.balanceAccountType === "Vendor" &&
    formData.balanceAccountNo &&
    balanceTdsSections.length > 0;
  const showBalanceTcs =
    formData.balanceAccountType === "Customer" &&
    formData.balanceAccountNo &&
  ...
  const formStateToVoucherData = (
    state: FormState,
  ): Partial<VoucherFormData> => {
    const data: Partial<VoucherFormData> = {
      ...
      accountType: state.accountType as
        | VoucherFormData["accountType"]
        | undefined,
    ...
    if (
      state.accountTdsSection &&
      state.accountTdsSection.tdsType &&
      state.accountTdsSection.tdsType !== "NA"
    ) {
    ...
    if (
      state.accountTcsSection &&
      state.accountTcsSection.tcsType &&
      state.accountTcsSection.tcsType !== "NA"
    ) {
    ...
    if (
      state.balanceTdsSection &&
      state.balanceTdsSection.tdsType &&
      state.balanceTdsSection.tdsType !== "NA"
    ) {
    ...
    if (
      state.balanceTcsSection &&
      state.balanceTcsSection.tcsType &&
      state.balanceTcsSection.tcsType !== "NA"
    ) {
  ...
  const validateAllFields = (
    formDataToValidate: FormState,
    attachmentsToValidate: File[] = attachments,
  ): ValidationErrors => {
    ...
    if (
      isEmployeeMandatory &&
      (!formDataToValidate.employee ||
        formDataToValidate.employee.trim() === "")
    ) {
      errors.employee = ["Employee is required"];
    }
    if (
      isAssignmentMandatory &&
      (!formDataToValidate.assignment ||
        formDataToValidate.assignment.trim() === "")
    ) {
    ...
    if (formDataToValidate.voucherType === "Cash Payment") {
      if (!attachmentsToValidate || attachmentsToValidate.length === 0) {
        errors.attachments = [
          "Attachment is required when Voucher Type is Cash Payment",
        ];
      }
    }
  ...
  const validateConditionalFields = (
    ...
    if (formDataToValidate.voucherType === "General Journal") {
      if (
        !formDataToValidate.externalDocumentNo ||
        formDataToValidate.externalDocumentNo.trim() === ""
      ) {
        errors.externalDocumentNo = [
      ...
      if (accountType === "Vendor" && formData.accountNo) {
        ...
        if (
          ...
          try {
            ...
            if (
              customer?.Assessee_Code &&
              customer.Assessee_Code.trim() !== ""
            ) {
      ...
      if (
        ...
        if (accountType === "Customer" && formData.accountNo) {
          try {
            ...
            console.error(
              "Error fetching Balance Account TDS sections:",
              error,
            );
            setBalanceTdsSections([]);
          }
  ...
  useEffect(() => {
    const fetchDimensions = async () => {
      ...
      try {
        ...
        const dimensionPromises = accountNumbers.map((no) =>
          getDefaultDimensions(no),
        );
        ...
        const employeeDim = allDimensions.find(
          (d) => d.Dimension_Code === "EMPLOYEE",
        );
        const assignmentDim = allDimensions.find(
          (d) => d.Dimension_Code === "ASSIGNMENT",
        );
  ...
  const loadVoucherForEdit = (voucher: VoucherEntryResponse) => {
    ...
    setTimeout(() => {
      setFormData({
        voucherType: getVoucherTypeFromTemplate(
          voucher.Journal_Template_Name,
        ) as "General Journal" | "Cash Receipt" | "Cash Payment" | undefined,
        ...
        documentType:
          ...
          (typeof voucher.Document_Type === "string" &&
            voucher.Document_Type.trim() === "")
            ? "NA"
            : (voucher.Document_Type as
                | "Payment"
                | "Invoice"
                | "Credit Memo"
                | "Refund"
        ...
        accountType: voucher.Account_Type as
          | "G/L Account"
          | "Customer"
          | "Vendor"
          | undefined,
        accountNo: voucher.Account_No,
        accountTdsSection: voucher.TDS_Section_Code
          ? { tdsType: voucher.TDS_Section_Code }
          : undefined,
        accountTcsSection: voucher.TCS_Nature_of_Collection
          ? { tcsType: voucher.TCS_Nature_of_Collection }
          : undefined,
        externalDocumentNo: voucher.External_Document_No || "",
        description: voucher.Description || undefined,
        amount: voucher.Amount.toString(),
        balanceAccountType: voucher.Bal_Account_Type as
          | "G/L Account"
          | "Customer"
          | "Vendor"
          | undefined,
  ...
  const handleUpdateVoucher = async () => {
    ...
    try {
      ...
      const hasChanged = (newVal: unknown, oldVal: unknown): boolean => {
        ...
        const normalizedOld =
          oldVal === "" || oldVal === null ? undefined : oldVal;
      ...
      if (hasChanged(data.externalDocumentNo, original.External_Document_No)) {
        updatePayload.External_Document_No =
          data.externalDocumentNo || undefined;
      }
      ...
      if (
        hasChanged(data.accountTdsSection?.tdsType, original.TDS_Section_Code)
      ) {
        updatePayload.TDS_Section_Code = data.accountTdsSection?.tdsType;
      }
      if (
        hasChanged(
          data.accountTcsSection?.tcsType,
          original.TCS_Nature_of_Collection,
        )
      ) {
        updatePayload.TCS_Nature_of_Collection =
          data.accountTcsSection?.tcsType;
      }
      ...
      // Success
      setSuccessMessage(
        `Voucher ${voucherToEdit.Document_No} updated successfully!`,
      );
      setSuccessDialogOpen(true);
      ...
      setErrorDialogData({
        ...
        message:
          "Failed to update voucher. Please review the error details below.",
        errors: [errorDetail],
        type: "general",
      });
  ...
  const handleDeleteVoucher = async (voucher: VoucherEntryResponse) => {
    ...
    try {
      ...
      setErrorDialogData({
        ...
        message:
          "Failed to delete voucher. Please review the error details below.",
        errors: [errorDetail],
        type: "general",
      });
      setErrorDialogOpen(true);
  ...
  const handlePostVouchers = async () => {
    ...
    try {
      ...
      setErrorDialogData({
        ...
        message:
          "Failed to post vouchers. Please review the error details below.",
        errors: [errorDetail],
        type: "post",
      });
      setErrorDialogOpen(true);
  ...
  useEffect(() => {
    ...
    setValidationErrors((prev) => {
      ...
      if (conditionalErrors.externalDocumentNo) {
        ...
        if (
          !shouldRequireExternalDoc ||
          (formData.externalDocumentNo &&
            formData.externalDocumentNo.trim() !== "")
        ) {
  ...
  const buildVoucherPayload = (
    ...
    const payload: Partial<CreateVoucherPayload> = {
      ...
      Document_Type:
        entry.documentType === "NA" ? "" : entry.documentType || "Payment",
  ...
  const hasError = (fieldName: string): boolean =>
    !!validationErrors[fieldName]?.length;
  ...
  return (
    <div className="flex w-full min-w-0 flex-col gap-3 p-4">
      ...
      {isEditingVoucher && (
        <div className="mb-2 rounded-md border border-yellow-500 bg-yellow-100 p-2 text-sm font-medium text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-200">
          Editing EDP record: {voucherToEdit?.Document_No} (Line No.{" "}
          {voucherToEdit?.Line_No})
        </div>
      )}
      <div
                    ...
                    <ContextMenu>
                      <ContextMenuTrigger asChild>
                        ...
                        >
                          ...
                          <TableCell className="px-1 py-0.5 text-xs">
                            {voucher.Shortcut_Dimension_1_Code || "-"}
                          </TableCell>
                          <TableCell className="px-1 py-0.5 text-xs">
                            {voucher.Shortcut_Dimension_2_Code || "-"}
                          </TableCell>
                          <TableCell className="px-1 py-0.5 text-xs">
                            {voucher.ShortcutDimCode3 || "-"}
                          </TableCell>
                          <TableCell className="px-1 py-0.5 text-xs">
                            {voucher.ShortcutDimCode4 || "-"}
                          </TableCell>
                          <TableCell className="px-1 py-0.5 text-xs">
                            {voucher.ShortcutDimCode5 || "-"}
                          </TableCell>
                          <TableCell className="px-1 py-0.5 text-xs">
                            {voucher.TDS_Section_Code || "-"}
                          </TableCell>
                          <TableCell className="px-1 py-0.5 text-xs">
                            {voucher.TCS_Nature_of_Collection || "-"}
                          </TableCell>
                          <TableCell className="px-1 py-0.5 text-xs">
                            <Tooltip
                              ...
                              onOpenChange={(open) => {
                                if (open) {
                                  ...
                                  if (
                                    !taxDetails[currentDocNo] &&
                                    !loadingTaxDetails[currentDocNo]
                                  ) {
                            ...
                            >
                              <TooltipTrigger asChild>
                                <button
                                  type="button"
                                  className="hover:bg-muted/50 flex h-full w-full items-center justify-center rounded p-1 transition-colors"
                                >
                                  <Info className="text-muted-foreground hover:text-foreground h-4 w-4" />
                                </button>
                              </TooltipTrigger>
                              <TooltipContent className="relative max-w-5xl">
                                <button
                                  type="button"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                  ...
                                  className="hover:bg-muted absolute top-1 right-1 z-10 rounded-sm p-1 transition-colors"
                                  aria-label="Close"
                                >
                                ...
                                {loadingTaxDetails[voucher.Document_No] ? (
                                ...
                                ) : taxDetails[voucher.Document_No] &&
                                  ...
                                  <div className="space-y-3 pr-8">
                                    ...
                                    <div className="max-h-96 space-y-2 overflow-y-auto text-sm">
                                      {taxDetails[voucher.Document_No].map(
                                        (detail, idx) => (
                                          <div
                                            key={idx}
                                ...
                                ) : taxDetails[voucher.Document_No]?.length ===
                                  0 ? (
      ...
      <Dialog
        open={showUpdateVoucherWarning}
        onOpenChange={setShowUpdateVoucherWarning}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Update Voucher</DialogTitle>
            <DialogDescription>
              Are you sure you want to update voucher{" "}
              {voucherToEdit?.Document_No} (Line No. {voucherToEdit?.Line_No})?
      ...
      <Dialog
        open={showDeleteVoucherWarning}
        onOpenChange={setShowDeleteVoucherWarning}
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {voucherToDelete?.Document_No}? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
