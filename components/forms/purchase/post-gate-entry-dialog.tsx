...
export function PostGateEntryDialog({
  ...
  const loadRows = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await getPostGateEntryLineList(sourceNo, "Inward");
      setRows(response);
      setSelectedIds(new Set());
    } catch (error) {
      toast.error(
        (error as ApiError).message ?? "Failed to load post gate entry lines.",
      );
      setRows([]);
      setSelectedIds(new Set());
    } finally {
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-xs">Line No</TableHead>
                  ...
                  <TableHead className="text-xs">Challan Date</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {isLoading ? (
                  ...
                  rows.map((row) => {
                    ...
                    return (
                      <TableRow key={id}>
                        ...
                        <TableCell className="text-xs">{row.Line_No}</TableCell>
                        <TableCell className="text-xs">
                          {row.Entry_Type || "-"}
                        </TableCell>
                        <TableCell className="text-xs">
