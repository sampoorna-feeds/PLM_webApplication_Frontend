...
import {
  getVendorLedgerEntriesForDialog,
...
const visibleColumns = DEFAULT_COLUMNS.map((c) => c.id);

export function ApplyVendorEntriesDialog({
  documentNo,
  vendorNo,
  onSuccess,
  disabled,
}: ApplyVendorEntriesDialogProps) {
  ...
  // Multi-select state
  const [selectedEntryMap, setSelectedEntryMap] = useState<
    Map<number, VendorLedgerEntry>
  >(new Map());
  ...
  // Derive final list: selected entries float to top, followed by unselected
  ...
  // Checkbox handlers
  const toggleSelect = (entry: VendorLedgerEntry) => {
    setSelectedEntryMap((prev) => {
      const newMap = new Map(prev);
      if (newMap.has(entry.Entry_No)) {
        newMap.delete(entry.Entry_No);
      } else {
        newMap.set(entry.Entry_No, entry);
      }
      return newMap;
    });
  };
  ...
  const toggleAll = (checked: boolean) => {
    if (!checked) {
      ...
      const newMap = new Map(selectedEntryMap);
      entries.forEach((e) => newMap.set(e.Entry_No, e));
      setSelectedEntryMap(newMap);
    }
  };
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      for (const entry of selectedArray) {
        await applyVendorLedgerEntry(documentNo, entry.Entry_No);
      }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" disabled={disabled || !vendorNo}>
          Apply Entries
        </Button>
      </DialogTrigger>
      <DialogContent
      ...
      >
        ...
        {/* ── Search bar ────────────────────────────────────────────── */}
        <div className="bg-muted/30 flex shrink-0 items-center justify-between border-b px-5 py-2.5">
              ...
              <tr>
                <th className="bg-muted m-0 w-10 border-b px-3 text-center align-middle">
                  <Checkbox
                    checked={
                      entries.length > 0 &&
                      entries.every((e) => selectedEntryMap.has(e.Entry_No))
                    }
            ...
            <tbody>
              {isLoading ? (
                ...
                <>
                  {displayList.map((entry, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-3 py-2.5 text-sm whitespace-nowrap tabular-nums">
                          {entry.Posting_Date || (
                            <span className="opacity-30">—</span>
                          )}
                        </td>
                        <td className="px-3 py-2.5 text-sm font-medium whitespace-nowrap tabular-nums">
                  ...
                  <tr
                    ref={sentinelRef}
                    className="m-0 h-1 border-0 bg-transparent p-0 text-transparent hover:bg-transparent"
                  >
                    <td
                      colSpan={DEFAULT_COLUMNS.length + 1}
        ...
        <div className="bg-muted/10 flex shrink-0 items-center justify-between border-t px-5 py-3.5">
          <div className="flex items-center gap-2">
            ...
            <span className="bg-primary/10 text-primary border-primary/20 rounded border px-2 py-1 text-xs font-semibold">
              {selectedEntryMap.size} selected
            </span>
          </div>
          <div className="flex items-center gap-2">
            <Button
            ...
            <Button
              onClick={handleApply}
              disabled={selectedEntryMap.size === 0 || isApplying}
              size="sm"
              className="h-8"
            >
              {isApplying ? (
...
// ----------------------------------------------------------------------
// Sub-components for Table Header
// ----------------------------------------------------------------------
    ...
    <Popover open={open} onOpenChange={setOpen} modal={false}>
      <PopoverTrigger asChild>
        <button
          type="button"
          className={cn(
            "hover:text-primary ml-auto transition-colors",
